...
func TestConvert(t *testing.T) {
	...
	tables := []struct {
		...
		{`SELECT * WHERE ip='10.10.10.10' LIMIT 5,10`, `ip = '10.10.10.10' OFFSET 5 LIMIT 10`},
		{`SELECT * WHERE size>100 ORDER BY name LIMIT 0,1`, `size > 100 order by name asc OFFSET 0 LIMIT 1`},
		{`SELECT * WHERE size=10 ORDER BY name DESC LIMIT 0,1`, `size = 10 order by name desc OFFSET 0 LIMIT 1`},
		...
		{`SELECT * WHERE name='sarah' and age!=40 AND (country='LV' OR country='AU') ORDER BY age DESC limit 1`,
			`name = 'sarah' and age != 40 and (country = 'LV' or country = 'AU') order by age desc OFFSET 0 LIMIT 1`},
	}
