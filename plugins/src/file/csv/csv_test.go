...
func TestConvert(t *testing.T) {
	...
	tables := []struct {
		...
		{`SELECT * WHERE ip='10.10.10.10' LIMIT 5,10`, `ip = '10.10.10.10' LIMIT 10 OFFSET 5`},
		{`SELECT * WHERE size>100 ORDER BY name LIMIT 0,1`, `size > 100 order by name asc LIMIT 1 OFFSET 0`},
		{`SELECT * WHERE size=10 ORDER BY name DESC LIMIT 0,1`, `size = 10 order by name desc LIMIT 1 OFFSET 0`},
		...
		{`SELECT * WHERE name='sarah' and age!=40 and (country='LV' OR country='AU') ORDER BY age DESC limit 1`,
			`name = 'sarah' and age != 40 and (country = 'LV' or country = 'AU') order by age desc LIMIT 1 OFFSET 0`},
	}
